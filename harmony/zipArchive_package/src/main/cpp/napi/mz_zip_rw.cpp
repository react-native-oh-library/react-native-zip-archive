...
typedef struct mz_zip_reader_s {
    ...
    funcProgressCallback funcProgress;
} mz_zip_reader;
...
int32_t mz_zip_reader_entry_save(void *handle, void *stream, mz_stream_write_cb write_cb) {
    ...
    if (reader->progress_cb) {
        reader->progress_cb(handle, reader->progress_userdata, reader->file_info, current_pos, progress, uncompressSize);
        if (reader->funcProgress) {
            reader->funcProgress(progress, uncompressSize);
        }
    }
...
void mz_zip_reader_set_progress_cb(void *handle, void *userdata, mz_zip_reader_progress_cb cb,
    ...
    reader->funcProgress = funcProgress;
}
...
typedef struct mz_zip_writer_s {
    ...
    uint8_t raw;
    uint8_t buffer[UINT16_MAX];
    funcProgressCallbackzip funcProgresszip;
} mz_zip_writer;
...
int32_t mz_zip_writer_entry_open(void *handle, mz_zip_file *file_info) {
    ...
    /* Open entry in zip */
    err =
        mz_zip_entry_write_open(writer->zip_handle, &writer->file_info, writer->compress_level, writer->raw, password);
...
int32_t mz_zip_writer_add(void *handle, void *stream, mz_stream_read_cb read_cb) {
    ...
    if (writer->progress_cb) {
        writer->progress_cb(handle, writer->progress_userdata, &writer->file_info, current_pos, progress);
        //writer->pcall(out);
    } if (writer->funcProgresszip) {
        writer->funcProgresszip(progress);
    }
...
void mz_zip_writer_set_progress_cb(void *handle, void *userdata, mz_zip_writer_progress_cb cb, funcProgressCallbackzip funcProgress) {
    mz_zip_writer *writer = (mz_zip_writer *)handle;
    writer->progress_cb = cb;
    writer->progress_userdata = userdata;
    writer->funcProgresszip = funcProgress;
}
